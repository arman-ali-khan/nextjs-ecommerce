...
const Draws = () => {
  ...
  let content;
  if (loading) {
    content = [...Array(10).keys()].map((product, i) => (
      <div
        key={i}
        className={`shadow-xl h-72 rounded-md bg-gray-300 animate-pulse`}
      >
        <label className="cursor-pointer">
          <div className={`h-56 bg-base-300 animate-pulse`}></div>

          <div className={`flex items-center justify-between px-3 `}>
            <div>
              <div className="flex justify-between items-center ">
                <h4 className="text-teal-600 font-bold">
                  <span className="text-gray-500 text-sm font-thin line-through"></span>
                </h4>
                <div></div>
              </div>
            </div>
  ...
  if (!loading && products?.length) {
    content = products?.map((product, i) => (
      <DrawCard key={i} product={product} />
    ));
  }
  return (
    <section className="container mx-auto md:my-12 my-3 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 md:px-0">
        <h2 className="md:text-2xl px-4 py-2 md:uppercase text-teal-600 flex items-center gap-2 border-b border-teal-600">
          {" "}
          <span>
            <BiTrendingUp />
          </span>
